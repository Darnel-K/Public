...
function init {
    ...
    $REG_HKU_PATH = "HKU:\.DEFAULT\Control Panel\International"
    $REG_HKCU_PATH = "HKCU:\Control Panel\International"
    $REG_DATA = @(
        [PSCustomObject]@{
            Path  = $REG_HKU_PATH
            Key   = "iFirstDayOfWeek"
    ...
    $CUSTOM_LOG.Information("Started Setting System Locale to United Kingdom.")
    # Mount HKEY_USERS registry hive
    try {
        New-PSDrive HKU Registry HKEY_USERS | Out-Null
        $CUSTOM_LOG.Success("Mounted 'HKEY_USERS' registry hive.")
    }
    catch {
        $CUSTOM_LOG.Fail("Failed to mount 'HKEY_USERS' registry hive.")
        $CUSTOM_LOG.Error($Error[0])
        Exit 1
    }

    # Check if Language Pack is installed and install if not
    $CUSTOM_LOG.Information("Checking if '$DESIRED_LANGUAGE' Language Pack is installed")
    if (-not (Get-WinUserLanguageList | Where-Object { $_.LanguageTag -eq $DESIRED_LANGUAGE })) {
        $CUSTOM_LOG.Information("$DESIRED_LANGUAGE Language Pack not installed")
        try {
            $CUSTOM_LOG.Information("Attempting to install $DESIRED_LANGUAGE Language Pack")
            Install-Language $DESIRED_LANGUAGE -CopyToSettings
            if (Get-WinUserLanguageList | Where-Object { $_.LanguageTag -eq $DESIRED_LANGUAGE }) {
                $CUSTOM_LOG.Success("$DESIRED_LANGUAGE Language Pack installed successfully")
            }
            else {
                $CUSTOM_LOG.Fail("Unable to install $DESIRED_LANGUAGE Language Pack")
                Exit 1
            }
        }
        catch {
        ...
        $CUSTOM_LOG.Information("$DESIRED_LANGUAGE Language Pack already installed")
    }

    # Check if SystemPreferredUILanguage is set to the Desired Language
    $CUSTOM_LOG.Information("Checking if SystemPreferredUILanguage is set to $DESIRED_LANGUAGE")
    if (-not ((Get-SystemPreferredUILanguage) -eq $DESIRED_LANGUAGE)) {
        $CUSTOM_LOG.Information("SystemPreferredUILanguage not set to $DESIRED_LANGUAGE")
        try {
            $CUSTOM_LOG.Information("Attempting to set SystemPreferredUILanguage to $DESIRED_LANGUAGE")
            Set-SystemPreferredUILanguage $DESIRED_LANGUAGE
            if ((Get-SystemPreferredUILanguage) -eq $DESIRED_LANGUAGE) {
                $CUSTOM_LOG.Success("SystemPreferredUILanguage set to $DESIRED_LANGUAGE successfully")
            }
            else {
                $CUSTOM_LOG.Fail("Unable to set SystemPreferredUILanguage to $DESIRED_LANGUAGE")
                Exit 1
            }
        }
        catch {
        ...
        $CUSTOM_LOG.Information("SystemPreferredUILanguage already set to $DESIRED_LANGUAGE")
    }

    # Check if WinSystemLocale is set to $DESIRED_LANGUAGE
    $CUSTOM_LOG.Information("Checking if WinSystemLocale is set to $DESIRED_LANGUAGE")
    if (-not ((Get-WinSystemLocale).Name -eq $DESIRED_LANGUAGE)) {
        $CUSTOM_LOG.Information("WinSystemLocale not set to $DESIRED_LANGUAGE")
        try {
            $CUSTOM_LOG.Information("Attempting to set WinSystemLocale to $DESIRED_LANGUAGE")
            Set-WinSystemLocale -SystemLocale $DESIRED_LANGUAGE
            if ((Get-WinSystemLocale).Name -eq $DESIRED_LANGUAGE) {
                $CUSTOM_LOG.Success("WinSystemLocale set to $DESIRED_LANGUAGE successfully")
            }
            else {
                $CUSTOM_LOG.Fail("Unable to set WinSystemLocale to $DESIRED_LANGUAGE")
                Exit 1
            }
        }
        catch {
        ...
        $CUSTOM_LOG.Information("WinSystemLocale already set to $DESIRED_LANGUAGE")
    }

    # Check if WinHomeLocation is set to United Kingdom
    $CUSTOM_LOG.Information("Checking if WinHomeLocation is set to United Kingdom")
    if (-not ((Get-WinHomeLocation).GeoId -eq 242)) {
        $CUSTOM_LOG.Information("WinHomeLocation not set to United Kingdom")
        try {
            $CUSTOM_LOG.Information("Attempting to set WinHomeLocation to United Kingdom")
            Set-WinHomeLocation -GeoId 242
            if ((Get-WinHomeLocation).GeoId -eq 242) {
                $CUSTOM_LOG.Success("WinHomeLocation set to United Kingdom successfully")
            }
            else {
                $CUSTOM_LOG.Fail("Unable to set WinHomeLocation to United Kingdom")
                Exit 1
            }
        }
        catch {
        ...
        $CUSTOM_LOG.Information("WinHomeLocation already set to United Kingdom")
    }

    # Uninstall other language packs
    $OtherLanguagePacks = (Get-InstalledLanguage) | Where-Object { $_.LanguageId -ne $DESIRED_LANGUAGE }
    foreach ($item in $OtherLanguagePacks) {
        try {
            Uninstall-Language -Language $item.LanguageId
            $CUSTOM_LOG.Success("Uninstalled '$($item.LanguageId)' Language Pack successfully")
        }
        catch {
            $CUSTOM_LOG.Fail("Unable to uninstall '$($item.LanguageId)' Language Pack")
            $CUSTOM_LOG.Error($Error[0])
        }
    }
    ...
    # Set user culture
    try {
        Set-Culture $DESIRED_LANGUAGE
        $CUSTOM_LOG.Success("Culture '$DESIRED_LANGUAGE' set successfully")
    }
    catch {
        $CUSTOM_LOG.Fail("Unable to set culture '$DESIRED_LANGUAGE'")
        $CUSTOM_LOG.Error($Error[0])
        Exit 1
    }
    ...
    try {
        ...
        $CUSTOM_LOG.Success("Copied locale settings to system")
    }
    catch {
        $CUSTOM_LOG.Fail("Unable to copy locale settings to system")
        $CUSTOM_LOG.Error($Error[0])
    }
    $CUSTOM_LOG.Success("Finished Setting System Locale to United Kingdom.")
    Exit 0
}
...
$SCRIPT_NAME = "Set-SystemLocale(UK)" # This is used in the window title and the event log entries.
