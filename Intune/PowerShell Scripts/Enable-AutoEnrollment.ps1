<#
# #################################################################################################################### #
# Filename: \Intune\PowerShell Scripts\Enable-AutoEnrollment.ps1                                                       #
# Repository: Public                                                                                                   #
# Created Date: Wednesday, August 13th 2025, 3:16:54 PM                                                                #
# Last Modified: Wednesday, August 13th 2025, 3:21:56 PM                                                               #
...
.DESCRIPTION
    Enables automatic enrollment into intune using user credentials
.EXAMPLE
    & .\Enable-AutoEnrollment.ps1
#>
...
function init {
    ...
    $REG_DATA = @(
        [PSCustomObject]@{
            Path  = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\MDM" # Registry key path
            Key   = "AutoEnrollMDM" # Registry property Key
            Value = "1" # Registry property value
            Type  = "DWord" # Binary, DWord, ExpandString, MultiString, String or QWord
        },
        [PSCustomObject]@{
            Path  = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\MDM" # Registry key path
            Key   = "UseAADCredentialType" # Registry property Key
            Value = "1" # Registry property value
            Type  = "DWord" # Binary, DWord, ExpandString, MultiString, String or QWord
        },
        [PSCustomObject]@{
            Path  = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\MDM" # Registry key path
            Key   = "MDMApplicationId" # Registry property Key
            Value = "" # Registry property value
            Type  = "String" # Binary, DWord, ExpandString, MultiString, String or QWord
        }
    )
...
$SCRIPT_NAME = "Enable-AutoEnrollment" # This is used in the window title and the event log entries.
