<#
# #################################################################################################################### #
# Filename: \Intune\Remediation Scripts\Docker-Users Local Group Membership\Detect-DockerUsersLocalGroupMembership.ps1 #
# Repository: Public                                                                                                   #
# Created Date: Saturday, January 4th 2025, 9:53:31 PM                                                                 #
# Last Modified: Saturday, January 4th 2025, 9:55:26 PM                                                                #
...
<#
.SYNOPSIS
    Detection script for remediation
.DESCRIPTION
    Detects if active user profiles are members of the docker-users group
.EXAMPLE
    & .\Detect-DockerUsersLocalGroupMembership.ps1
#>
        ...
        if ($user_profiles.Count -gt 0) {
            ...
            if ((Get-LocalGroupMember -Group "docker-users").Count -EQ $user_profiles.Count) {
                ...
                if ((Compare-Object -ReferenceObject $user_profiles -DifferenceObject (Get-LocalGroupMember -Group "docker-users") -Property SID -IncludeEqual | Where-Object -Property SideIndicator -EQ "==").Count -eq $user_profiles.Count) {
                    ...
                    $CUSTOM_LOG.Fail("Group membership does not match active user profiles")
                    $CUSTOM_LOG.Warning("Remediation Required!")
                    Exit 1
                }
            }
            else {
                $CUSTOM_LOG.Fail("Group membership and user profile count does not match")
                $CUSTOM_LOG.Warning("Remediation Required!")
                Exit 1
            }
        }
        else {
            $CUSTOM_LOG.Warning("Found '$($user_profiles.Count)' user profiles")
            $CUSTOM_LOG.Information("No further action required.")
            Exit 0
        }
    }
