...
# Last Modified: Sunday, January 12th 2025, 10:56:23 PM                                                                #
...
function init {
    ...
    $CUSTOM_LOG.Information("Setting all '$NET_TYPE_TO_DETECT' profiles to '$NET_TYPE_TO_SET'")
    ...
    if ($net_connection_profiles.Count -gt 0) {
        $CUSTOM_LOG.Information("Detected $($net_connection_profiles.Count) $NET_TYPE_TO_DETECT Profiles")
        Exit 1
    }
    else {
            $CUSTOM_LOG.Information("Detected 0 $NET_TYPE_TO_DETECT Profiles")
        Exit 0
    }
}
...
$SCRIPT_NAME = "Fix-NetConnectionProfile" # This is used in the window title and the event log entries.
